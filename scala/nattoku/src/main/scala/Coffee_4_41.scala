object Coffee_4_41 {
  ...
  // 分からなかった1番、問題の正答
  def largerThan(n: Int): Int => Boolean = {
    i => i > n
  }
  ...
  // 2番 new
  def devideBy(n: Int): Int => Boolean = {
    i => i % n == 0
  }
  ...
  // 3番 new
  def underLengthWords(n: Int): String => Boolean = {
    word => word.length < n
  }
  ...
  // 4番 new
  def countOfS(n: Int): String => Boolean = {
    word => word.size - word.replaceAll("s", "").size >= n
  }
